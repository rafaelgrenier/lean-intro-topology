...
instance : PartialOrder (X × X) where
  ...
  le_antisymm := by
    dsimp [LE.le, r]
    simp [Prod.lex_def]
    rintro a b c d ((altc | ⟨aeqc, bltd⟩) | ⟨hac, hbd⟩) ((clta | ⟨ceqa, dltb⟩) | ⟨hca, hdb⟩)
    · exact False.elim (lt_irrefl a (lt_trans altc clta))
    · refine False.elim (lt_irrefl a ?_)
      nth_rw 2 [←ceqa]
      exact altc
    · rw [hca, hdb]
      trivial
    · refine False.elim (lt_irrefl a ?_)
      nth_rw 1 [aeqc]
      exact clta
    · exact False.elim (lt_irrefl b (lt_trans bltd dltb))
    · rw [hca, hdb]
      trivial
    · rw [hac, hbd]
      trivial

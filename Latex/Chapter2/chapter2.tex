...
type and \lean{p} has type \lean{$\alpha\;\to$ Prop}, then 
\lean{$\forall$x:$\alpha$, p x} is the proposition that every term of type $\alpha$
satisifes \lean{p} and \lean{$\exists$x:$\alpha$, p x} is the proposition that
at least one term of type $\alpha$ satisfies \lean{p}. $\forall$ is a dependent
...
all terms of the given type, which Lean denotes \lean{Set.univ}.
...
tactic \lean{dsimp[Surjective]} or \lean{dsimp[Injective]}.
